...
void CGraphPanel::AppendMenuItems(CMenu* menu)
{
	...
	CMenu sub_menu;
	sub_menu.CreateMenu();
	menu->AppendMenu(MF_POPUP | MF_ENABLED | MF_STRING, (UINT_PTR)sub_menu.m_hMenu, _T("Zoom"));
	//Zoom tool
	menu_flags = MF_STRING | MF_ENABLED;
	if (bZoomActive)
	{
	...
	//Fit
	sub_menu.CreateMenu();
	menu->AppendMenu(MF_POPUP | MF_ENABLED | MF_STRING, (UINT_PTR)sub_menu.m_hMenu, _T("Fit..."));
...
void CGraphPanel::OnRBMenuCommand(UINT command_id)
{
	...
	case GRAPH_RBMC_VIEW_PROPERTIES:
	{
		...
		UINT_PTR result = prop_sheet.DoModal();
