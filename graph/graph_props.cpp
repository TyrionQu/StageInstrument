...
void CAxisProps::CalculateFormatStrings()
{
	...
	formatStrings[0] = new TCHAR[nSize];
	_tcscpy_s(formatStrings[0], nSize, (LPCTSTR)str);

	str.Format(_T("%%0.%df %s"), m_Precision, m_UOM);
	nSize = str.GetLength() + 1;
	formatStrings[1] = new TCHAR[nSize];
	_tcscpy_s(formatStrings[1], nSize, (LPCTSTR)str);

	str.Format(_T("%s: %%0.%df %s"), m_Title, m_Precision, m_UOM);
	nSize = str.GetLength() + 1;
	formatStrings[2] = new TCHAR[nSize];
	_tcscpy_s(formatStrings[2], nSize, (LPCTSTR)str);
}
